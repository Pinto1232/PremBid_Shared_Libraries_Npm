...
export const mutationKey = ["IndividualBiddingProfileUpdate"];

export const individualBiddingProfileUpdate = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>(
    {
      path: apiPaths.profile.individualBiddingProfileUpdate,
      payload: params,
    },
    requestOptionsInit
  );
...
export const useIndividualBiddingProfileUpdate = (options?: UseMutationOptions<Response, Error, Params>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const mutation = useMutation({
    mutationFn: (params: Params) => individualBiddingProfileUpdate(params, { userAuth, baseUrl }, requestOptionsInit),
    mutationKey,
    meta: {
      createdAt: Date.now(),
    },
