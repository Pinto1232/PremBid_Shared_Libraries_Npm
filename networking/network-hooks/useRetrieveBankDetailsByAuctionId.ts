...
type PathParams = { auctionId: string };

export type Params = QueryArgs<QueryParams, PathParams>;

export const queryKey = "RetrieveBankDetailsByAuctionId";

export const retrieveBankDetailsByAuctionId = async ({ queryParams, pathParams }: Params = {}, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).get<QueryParams, PathParams>({ path: apiPaths.vendor.retrieveBankDetailsByAuctionId, queryParams, pathParams }, requestOptionsInit);
};

export const useRetrieveBankDetailsByAuctionId = (params: Params, options?: UseQueryOptions<Response, Error>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const { data, ...rest } = useQuery({
    ...
    queryFn: () => retrieveBankDetailsByAuctionId(params, { userAuth, baseUrl }, requestOptionsInit),
    meta: {
      createdAt: Date.now(),
    },
    onError: (error: any) => {
