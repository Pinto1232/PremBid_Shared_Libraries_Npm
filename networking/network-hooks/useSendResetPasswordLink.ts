...
export const mutationKey = ["SendResetPasswordLink"];

export const sendResetPasswordLink = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>(
    {
      path: apiPaths.account.sendResetPasswordLink,
      payload: params,
    },
    requestOptionsInit
  );
...
export const useSendResetPasswordLink = (options?: UseMutationOptions<Response, Error, Params>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const mutation = useMutation({
    mutationFn: (params: Params) => sendResetPasswordLink(params, { userAuth, baseUrl }, requestOptionsInit),
    mutationKey,
    meta: {
      createdAt: Date.now(),
    },
