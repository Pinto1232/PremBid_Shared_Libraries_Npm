...
export const mutationKey = ["UpsertTermsAcceptance"];

export const upsertTermsAcceptance = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>(
    {
      path: apiPaths.account.upsertTermsAcceptance,
      payload: params,
    },
    requestOptionsInit
  );
...
export const useUpsertTermsAcceptance = (options?: UseMutationOptions<Response, Error, Params>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const mutation = useMutation({
    mutationFn: (params: Params) => upsertTermsAcceptance(params, { userAuth, baseUrl }, requestOptionsInit),
    mutationKey,
    meta: {
      createdAt: Date.now(),
    },
