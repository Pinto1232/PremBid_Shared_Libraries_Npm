...
export const mutationKey = ["UpdateReservePrice"];

export const updateReservePrice = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>(
    {
      path: apiPaths.lot.updateReservePrice,
      payload: params,
    },
    requestOptionsInit
  );
...
export const useUpdateReservePrice = (options?: UseMutationOptions<Response, Error, Params>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const mutation = useMutation({
    mutationFn: (params: Params) => updateReservePrice(params, { userAuth, baseUrl }, requestOptionsInit),
    mutationKey,
    meta: {
      createdAt: Date.now(),
    },
