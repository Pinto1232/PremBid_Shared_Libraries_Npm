...
export const queryKey = "RetrieveCompanyBiddingProfile";

export const retrieveCompanyBiddingProfile = async ({ queryParams, pathParams }: Params = {}, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).get<QueryParams, PathParams>({ path: apiPaths.profile.retrieveCompanyBiddingProfile, queryParams, pathParams }, requestOptionsInit);
};

export const useRetrieveCompanyBiddingProfile = (params: Params, options?: UseQueryOptions<Response, Error>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const { data, ...rest } = useQuery({
    ...
    queryFn: () => retrieveCompanyBiddingProfile(params, { userAuth, baseUrl }, requestOptionsInit),
    meta: {
      createdAt: Date.now(),
    },
    onError: (error: any) => {
