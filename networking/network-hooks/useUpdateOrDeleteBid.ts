...
export const mutationKey = ["UpdateOrDeleteBid"];

export const updateOrDeleteBid = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>(
    {
      path: apiPaths.bids.updateOrDeleteBid,
      payload: params,
    },
    requestOptionsInit
  );
...
export const useUpdateOrDeleteBid = (options?: UseMutationOptions<Response, Error, Params>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const mutation = useMutation({
    mutationFn: (params: Params) => updateOrDeleteBid(params, { userAuth, baseUrl }, requestOptionsInit),
    mutationKey,
    meta: {
      createdAt: Date.now(),
    },
