...
export const queryKey = "RetrieveLotDetailsPageData";

export const retrieveLotDetailsPageData = async ({ queryParams, pathParams }: Params = {}, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).get<QueryParams, PathParams>({ path: apiPaths.lots.retrieveLotDetailsPageData, queryParams, pathParams }, requestOptionsInit);
};

export const useRetrieveLotDetailsPageData = (params: Params, options?: UseQueryOptions<Response, Error>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const { data, ...rest } = useQuery({
    ...
    queryFn: () => retrieveLotDetailsPageData(params, { userAuth, baseUrl }, requestOptionsInit),
    meta: {
      createdAt: Date.now(),
    },
    onError: (error: any) => {
