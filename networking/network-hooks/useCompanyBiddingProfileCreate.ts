...
export const mutationKey = ["CompanyBiddingProfileCreate"];

export const companyBiddingProfileCreate = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>(
    {
      path: apiPaths.profile.companyBiddingProfileCreate,
      payload: params,
    },
    requestOptionsInit
  );
...
export const useCompanyBiddingProfileCreate = (options?: UseMutationOptions<Response, Error, Params>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const mutation = useMutation({
    mutationFn: (params: Params) => companyBiddingProfileCreate(params, { userAuth, baseUrl }, requestOptionsInit),
    mutationKey,
    meta: {
      createdAt: Date.now(),
    },
