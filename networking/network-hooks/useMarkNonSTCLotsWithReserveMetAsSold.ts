...
export const mutationKey = ["MarkNonSTCLotsWithReserveMetAsSold"];

export const markNonSTCLotsWithReserveMetAsSold = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>(
    {
      path: apiPaths.lots.MarkNonSTCLotsWithReserveMetAsSold,
      payload: params,
    },
    requestOptionsInit
  );
...
export const useMarkNonSTCLotsWithReserveMetAsSold = (options?: UseMutationOptions<Response, Error, Params>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const mutation = useMutation({
    mutationFn: (params: Params) => markNonSTCLotsWithReserveMetAsSold(params, { userAuth, baseUrl }, requestOptionsInit),
    mutationKey,
    meta: {
      createdAt: Date.now(),
    },
