...
export const queryKey = "RetrieveAllMyBiddingActivity";

export const retrieveAllMyBiddingActivity = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>({ path: apiPaths.lotWishlist.retrieveAllMyBiddingActivity, payload: params }, requestOptionsInit);
};

export const useRetrieveAllMyBiddingActivity = (params: Params, options?: UseQueryOptions<Response, Error>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const { data, ...rest } = useQuery({
    ...
    queryFn: () => retrieveAllMyBiddingActivity(params, { userAuth, baseUrl }, requestOptionsInit),
    meta: {
      createdAt: Date.now(),
    },
    onError: (error: any) => {
