...
export const mutationKey = ["RemoveAccountDeletion"];

export const removeAccountDeletion = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>(
    {
      path: apiPaths.account.removeAccountDeletion,
      payload: params,
    },
    requestOptionsInit
  );
...
export const useRemoveAccountDeletion = (options?: UseMutationOptions<Response, Error, Params>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const mutation = useMutation({
    mutationFn: (params: Params) => removeAccountDeletion(params, { userAuth, baseUrl }, requestOptionsInit),
    mutationKey,
    meta: {
      createdAt: Date.now(),
    },
