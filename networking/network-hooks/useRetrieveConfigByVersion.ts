...
type Response = ClientConfig;

type QueryParams = { Version: number };

type PathParams = undefined;

export type Params = QueryArgs<QueryParams, PathParams>;

export const queryKey = "RetrieveConfigByVersion";

export const retrieveConfigByVersion = async ({ queryParams, pathParams }: Params = {}, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).get<QueryParams, PathParams>({ path: apiPaths.vendor.retrieveConfigByVersion, queryParams, pathParams }, requestOptionsInit);
};

export const useRetrieveConfigByVersion = (params: Params, options?: UseQueryOptions<Response, Error>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const { data, ...rest } = useQuery({
    ...
    queryFn: () => retrieveConfigByVersion(params, { userAuth, baseUrl }, requestOptionsInit),
    staleTime: cacheTime,
    cacheTime,
    retry: true,
    meta: {
