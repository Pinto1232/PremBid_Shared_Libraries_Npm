...
export const queryKey = "RetrieveProfileDocuments";

export const retrieveProfileDocuments = async ({ queryParams, pathParams }: Params = {}, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).get<QueryParams, PathParams>({ path: apiPaths.document.retrieveProfileDocuments, queryParams, pathParams }, requestOptionsInit);
};

export const useRetrieveProfileDocuments = (params: Params, options?: UseQueryOptions<Response, Error>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const { data, ...rest } = useQuery({
    ...
    queryFn: () => retrieveProfileDocuments(params, { userAuth, baseUrl }, requestOptionsInit),
    meta: {
      createdAt: Date.now(),
    },
    onError: (error: any) => {
