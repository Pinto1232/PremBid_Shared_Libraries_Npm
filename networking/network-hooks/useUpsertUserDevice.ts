...
export const mutationKey = ["UpsertUserDevice"];

export const upsertUserDevice = async (params: Params, { userAuth, baseUrl }: RequestConfig, requestOptionsInit?: RequestOptionsInit): Promise<Response> => {
  return await requestClient({ userAuth, baseUrl }).post<Params>(
    {
      path: apiPaths.auction.upsertUserDevice,
      payload: params,
    },
    requestOptionsInit
  );
...
export const useUpsertUserDevice = (options?: UseMutationOptions<Response, Error, Params>, requestOptionsInit?: RequestOptionsInit) => {
  ...
  const mutation = useMutation({
    mutationFn: (params: Params) => upsertUserDevice(params, { userAuth, baseUrl }, requestOptionsInit),
    mutationKey,
    meta: {
      createdAt: Date.now(),
    },
